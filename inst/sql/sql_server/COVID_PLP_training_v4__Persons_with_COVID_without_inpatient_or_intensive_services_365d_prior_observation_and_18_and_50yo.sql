...
WHERE (C.condition_start_date >= DATEFROMPARTS(2020, 1, 1) and C.condition_start_date <= DATEFROMPARTS(2020, 8, 1))
...
WHERE (C.measurement_date >= DATEFROMPARTS(2020, 1, 1) and C.measurement_date <= DATEFROMPARTS(2020, 8, 1))
...
WHERE (C.observation_date >= DATEFROMPARTS(2020, 1, 1) and C.observation_date <= DATEFROMPARTS(2020, 8, 1))
AND C.value_as_concept_id in (4126681,45877985,9191,45884084,4181412,45879438)
...
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.END_DATE >= DATEADD(day,-30,P.START_DATE) AND A.END_DATE <= DATEADD(day,0,P.START_DATE) ) cc on p.person_id = cc.person_id and p.event_id = cc.event_id
GROUP BY p.person_id, p.event_id
HAVING COUNT(cc.event_id) = 0
-- End Correlated Criteria
...
WHERE (YEAR(E.start_date) - P.year_of_birth >= 18 and YEAR(E.start_date) - P.year_of_birth <= 50)
GROUP BY e.person_id, e.event_id
